...
def login():
    ...
    # 输入查询内容
    driver.find_element(by='xpath', value='//*[@id="q"]').send_keys('java')
    # 获取到搜索按钮后，点击
    ...
    # 点击二维码扫描
    driver.find_element(by='xpath', value='//*[@id="login"]/div[1]/i').click()
    # 等待扫描二维码，时间短了就改一改
    time.sleep(10)
...
def get_gooods(serach):
    ...
    divs = driver.find_elements(by='xpath', value=
    ...
    for div in divs:
        ...
        xh += 1
        # 获取图片地址
        img = div.find_element(by='xpath', value=
        ...
        # 获取标题
        title = div.find_element(by='xpath', value='//*[@id="J_Itemlist_TLink_709786641391"]/text()').text
        # 获取价格
        price = div.find_element(by='xpath', value=
        '//*[@id="mainsrp-itemlist"]/div/div/div[1]/div[1]/div[2]/div[1]/div[1]/strong').text + '元'
        # 付款人数（销售数）
        fukuan = div.find_element(by='xpath',
                                  value='//*[@id="mainsrp-itemlist"]/div/div/div[1]/div[1]/div[2]/div[1]/div[2]').text
        # 一条数据
...
def to_searchs(serachs):
    for serach in serachs:
        print(serach)
        # 获取文本框
        serachInput = driver.find_element(by='xpath', value='//*[@id="q"]')
        # 清空文本框
        serachInput.clear()
        # 输入查询内容
        serachInput.send_keys(serach)
        # 点击搜索按钮  //*[@id="J_SearchForm"]/button
        driver.find_element(by='xpath', value='//*[@id="J_SearchForm"]/button').click()
        # 拉动侧边滑动条，使页面数据加载完全
        drop_scroll()
        # 获取商品信息
        get_gooods(serach)
...
if __name__ == "__main__":
    ...
    datas = []
    # 查找的关键词集合
    searchs = {'java语言', 'python', 'c++', '安卓开发', 'JavaScript', 'html5', 'vue', '人工智能与机器学习', '数据结构',
               'node.js', 'linux', '计算机组成原理', '汇编原理', 'swift语言', 'go语言'}
    # 扫码登录函数
    login()
    # 查找所有商品信息
    to_searchs(searchs)
